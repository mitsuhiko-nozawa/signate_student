...
class NN_Model(BaseModel):
    ...
    def fit(self, train_X, train_y, valid_X, valid_y):
        ...
        if self.scheduler == "OneCycleLR":
            ...
            self.scheduler_param["steps_per_epoch"] = len(trainloader)
    ...
    def parse_param(self, params):
        self.DEVICE = torch.device("cuda:8") if torch.cuda.is_available() else torch.device("cpu")
        print(self.DEVICE)
...
class Base_NN(nn.Module):
    def __init__(self, embedding_dim, max_seq_len, n_embfeats, n_numfeats):
        ...
        self.h3 = self.n_embfeats + self.n_numfeats
        self.h4 = self.n_embfeats + self.n_numfeats
    ...
    def forward(self, x1, x2):
        ...
        x1_ = self.bn1(x1)
        x1_ = self.dropout1(x1_)
        x1_ = self.dense1(x1_)
        x1 = self.relu1(x1+x1_)

        x2_ = self.bn2(x2)
        x2_ = self.dropout2(x2_)
        x2_ = self.dense2(x2_)
        x2 = self.relu2(x2+x2_)
